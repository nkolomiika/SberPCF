...
import { Avatar, Box, Button, Card, CardContent, Checkbox, Chip, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, FormControlLabel, IconButton, Menu, MenuItem, Stack, TextField, Tooltip, Typography, } from "@mui/material";
...
export function UsersAdminPage() {
    ...
    const filteredUsers = useMemo(() => {
        ...
        return users.filter((user) => [user.username, user.full_name || "", user.email, roleLabel(user.role), ...user.tags].some((value) => value.toLowerCase().includes(query)));
    }, [searchQuery, users]);
    ...
    const handleResetPassword = async () => {
        ...
        try {
            ...
            setInfoMessage(result.mail_preview_url
    ...
    return (_jsxs(Stack, { spacing: 2.5, children: [_jsxs(Stack, { direction: { xs: "column", sm: "row" }, justifyContent: "space-between", alignItems: { sm: "center" }, gap: 1.5, children: [_jsxs(Stack, { spacing: 0.3, children: [_jsx(Typography, { variant: "h4", fontWeight: 700, children: "\u041F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u0438" }), _jsxs(Stack, { direction: "row", spacing: 1, flexWrap: "wrap", useFlexGap: true, children: [_jsx(Chip, { label: `Всего: ${users.length}`, variant: "outlined" }), _jsx(Chip, { label: `Активных: ${activeUsers}`, variant: "outlined" }), _jsx(Chip, { label: `Показано: ${filteredUsers.length}`, variant: "outlined" })] })] }), _jsx(Tooltip, { title: "\u0414\u0435\u0439\u0441\u0442\u0432\u0438\u044F", children: _jsx(IconButton, { onClick: (event) => setPageActionsAnchorEl(event.currentTarget), sx: { border: "1px solid rgba(126,224,255,0.18)" }, children: _jsx(MoreVertIcon, {}) }) })] }), _jsx(Menu, { anchorEl: pageActionsAnchorEl, open: Boolean(pageActionsAnchorEl), onClose: closePageActions, anchorOrigin: { vertical: "bottom", horizontal: "right" }, transformOrigin: { vertical: "top", horizontal: "right" }, children: _jsx(MenuItem, { onClick: () => {
                        closePageActions();
                        setCreateOpen(true);
                    }, children: "\u0421\u043E\u0437\u0434\u0430\u0442\u044C \u043F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u044F" }) }), _jsx(TextField, { placeholder: "\u041F\u043E\u0438\u0441\u043A \u043F\u043E \u0438\u043C\u0435\u043D\u0438, \u043B\u043E\u0433\u0438\u043D\u0443, email, \u0440\u043E\u043B\u0438 \u0438\u043B\u0438 \u0442\u0435\u0433\u0443", value: searchQuery, onChange: (event) => setSearchQuery(event.target.value), fullWidth: true }), _jsxs(Stack, { spacing: 1.5, children: [filteredUsers.map((user) => (_jsx(Card, { sx: {
                        ...
                        }, children: _jsx(CardContent, { sx: { p: 1.25 }, children: _jsxs(Stack, { direction: { xs: "column", md: "row" }, justifyContent: "space-between", gap: 1, children: [_jsxs(Stack, { spacing: 0.45, sx: { minWidth: 0 }, children: [_jsxs(Stack, { direction: "row", spacing: 1, alignItems: "center", children: [_jsx(Avatar, { src: user.avatar_url ?? undefined, sx: { width: 32, height: 32, fontSize: 14 }, children: (user.full_name || user.username)[0]?.toUpperCase() }), _jsxs(Stack, { spacing: 0.15, sx: { minWidth: 0 }, children: [_jsx(Typography, { variant: "body1", fontWeight: 600, noWrap: true, children: user.full_name || user.username }), _jsxs(Typography, { variant: "caption", color: "text.secondary", noWrap: true, children: ["@", user.username] })] })] }), _jsxs(Stack, { direction: "row", spacing: 0.75, alignItems: "center", flexWrap: "wrap", useFlexGap: true, children: [_jsx(Chip, { size: "small", label: roleLabel(user.role) }), _jsx(Chip, { size: "small", label: user.is_active ? "Активен" : "Отключен", color: user.is_active ? "success" : "default", variant: user.is_active ? "filled" : "outlined" }), user.must_change_password && _jsx(Chip, { size: "small", color: "warning", label: "\u0422\u0440\u0435\u0431\u0443\u0435\u0442\u0441\u044F \u0441\u043C\u0435\u043D\u0430 \u043F\u0430\u0440\u043E\u043B\u044F" }), user.tags.map((tag) => (_jsx(Chip, { size: "small", variant: "outlined", label: tag }, `${user.id}-${tag}`)))] }), _jsx(Typography, { variant: "body2", color: "text.secondary", noWrap: true, children: user.email }), _jsxs(Typography, { variant: "caption", color: "text.secondary", children: ["\u0421\u043E\u0437\u0434\u0430\u043D: ", new Date(user.created_at).toLocaleString()] })] }), _jsx(Stack, { direction: "row", spacing: 0.5, alignItems: "flex-start", children: _jsx(IconButton, { className: "user-actions-trigger", size: "small", onClick: (event) => {
                                                setActiveUser(user);
                                                setUserActionsAnchorEl(event.currentTarget);
                                            }, children: _jsx(MoreVertIcon, { fontSize: "small" }) }) })] }) }) }, user.id))), filteredUsers.length === 0 && _jsx(Typography, { color: "text.secondary", children: "\u041F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u0438 \u043D\u0435 \u043D\u0430\u0439\u0434\u0435\u043D\u044B." })] }), _jsxs(Menu, { anchorEl: userActionsAnchorEl, open: Boolean(userActionsAnchorEl), onClose: closeUserActions, anchorOrigin: { vertical: "bottom", horizontal: "right" }, transformOrigin: { vertical: "top", horizontal: "right" }, children: [_jsxs(MenuItem, { onClick: () => {
                            if (activeUser) {
