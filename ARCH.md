...
Глобальное состояние — через React Context. Логика доступа — в custom hooks.

```typescript
// store/WebSocketContext.tsx
const WebSocketContext = createContext<WebSocketContextValue | null>(null);

// hooks/useWebSocket.ts
export function useWebSocket(projectId: string) {
  const ws = useRef<WebSocket | null>(null);
  // Подключение, переподключение, обработка событий
  ...
}
```
...
> Все эндпоинты раздела доступны только роли `admin`, кроме GET /me.
...
Сбросить/изменить пароль пользователя.
...
**Query params:** `?page=1&size=50&user_id=uuid&action=LOGIN&entity_type=vulnerability&from=2024-01-01&to=2024-12-31`
