...
```json
{
  ...
  "role": "pentester"        // "admin" | "pentester" | "developer", default "pentester"
}
```
...
- `pentester` и `developer` видят только проекты, в которых являются участниками
...
```json
{
  ...
  "workflow_steps": [
    {
      "id": "step-1",                  // required, непустая строка
      "title": "Подготовка запроса",   // required, непустая строка
      "image_file_ids": ["uuid"],      // optional
      "endpoint_id": "uuid",           // optional
      "endpoint_request_raw": "..."    // optional
    }
  ],
...
Примечание по текущему UI: при создании этапов поле `description` в форме не используется; этап сохраняется по обязательному `title` и дополнительным полям (скриншоты/endpoint-привязка).  
Ошибка вида `workflow_steps.N.title: Поле не может быть пустым` означает, что у этапа передан пустой `title`.
...
- шаги воспроизведения, включая структурированные `workflow_steps` (этапы с обязательным `title` и вложениями), если они заданы
...
## 3.17 Mail Delivery — Очередь писем

Сервис отправки email реализован асинхронно и отделён от HTTP-запросов:

- API создаёт задание в `mail_jobs` со статусом `pending`;
- задача публикуется в очередь RabbitMQ (`mail_queue_name`);
- отдельный `mail-worker` читает очередь и отправляет письмо через SMTP;
- при ошибке используется повторная попытка до `mail_max_attempts`;
- при успехе статус задачи меняется на `sent`.

Это используется, в том числе, для сценариев сброса пароля и приглашений с временным паролем.

---

# 4. Матрица доступа к эндпоинтам

| Эндпоинт | admin | member (`pentester`/`developer`) | не участник |
|---|:---:|:---:|:---:|
| POST /auth/login | ✓ | ✓ | ✓ |
| POST /auth/refresh | ✓ | ✓ | ✓ |
...
# App
DEBUG=false
BACKEND_CORS_ORIGINS=https://localhost:3000,https://127.0.0.1:3000
CSRF_ALLOWED_ORIGINS=https://localhost:3000,https://127.0.0.1:3000

# Mail queue + SMTP
RABBITMQ_URL=amqp://guest:guest@rabbitmq/
MAIL_QUEUE_NAME=pcf.mail
MAIL_ENABLED=true
MAIL_MAX_ATTEMPTS=5
SMTP_HOST=mailpit
SMTP_PORT=1025
