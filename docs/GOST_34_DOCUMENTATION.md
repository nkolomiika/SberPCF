# Документация на автоматизированную систему «PCF — Pentest Collaboration Framework»
...
Применительно к проекту PCF документация группируется следующим образом
...
Полное наименование: **Pentest Collaboration Framework** (PCF) — корпоративная
платформа совместной работы команды пентестеров.

Краткое наименование: **PCF**.
...
PCF — корпоративная закрытая платформа для централизованного ведения работ
...
- БД PostgreSQL — резервируется по корпоративной схеме (вне scope PCF);
- MinIO — резервируется по корпоративной схеме (вне scope PCF);
...
1. **Структурированные данные** — PostgreSQL (база данных PCF):
...
flowchart TD
    ...
    H --> I[audit LOGIN]
    I --> J([LoginResponse:<br/>id, username, role, must_change_password])
```
