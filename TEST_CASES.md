...
**Ожидаемый результат:**
- Статус: `200 OK`
- Поля: `id`, `username`, `email`, `role`, `is_active`, `created_at`
...
### TC-USR-008 🟡 Пароль короче 8 символов
...
### TC-NAV-004 🟢 Раздел «Уязвимости проекта» на уровне проекта

**Приоритет:** Высокий

**Шаги:**
1. В левой панели выбрать `Уязвимости проекта`
2. Создать уязвимость

**Ожидаемый результат:**
- Уязвимость создаётся в рамках проекта
- Раздел не привязан к одному конкретному хосту
- В карточке уязвимости доступны действия по привязке активов через модуль VULN ASSETS

---

### TC-NAV-005 🟢 Гибридный режим sidebar (свернуть/развернуть)
...
**Шаги:**
1. GET `/api/v1/audit-logs?user_id={id}&from=2024-01-01&to=2024-12-31`
...
**Предусловия:** `pentester2` подключён по WS
...
```json
{
  ...
  "data": {
    "type": "mention",
    "is_read": false,
    "context": {"vulnerability_title": "...", "commenter_username": "pentester1"}
  }
}
```
